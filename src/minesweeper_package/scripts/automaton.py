...
def launch_simulation(strategy, environment, duration=1800, repetitions=10, delay=25):
    ...
    for iteration in range(repetitions):
        ...
        try:
            gazebo_path = (
                f"{packager.get_path('turtlebot3_gazebo')}/launch/{environment}.launch"
            )
        except:
            sys.exit("The specified environment couldn't be found")
        try:
            driver_path = (
                f"{packager.get_path('minesweeper_package')}/launch/{strategy}.launch"
            )
        except:
        ...
        if "smart" in strategy:
            ...
            slam_launcher.start()
            time.sleep(delay)
            navigation_launcher = roslaunch.parent.ROSLaunchParent(
...
if __name__ == "__main__":
    ...
    environments = sorted([name[:-7] for name in files])[
        0:6
    ]  # Only show the 6 ones we've created
    print(environments)
    ...
    parser = argparse.ArgumentParser(
        description="Automatically run simulations one after the other"
    )
    parser.add_argument(
        "strategy",
