...
class SmartRoomba(RobotSlamController):
        ...
        if len(options) > 0:
            ...
            if self.strategy == "random":
                ...
                reciprocal_distances = 1 / (
                    np.sum((options - self.position_on_map) ** 2, -1) + 1e-6
                )
    ...
    def __plot_map(self):
        ...
        if self.map_update:
            ...
            ax.scatter(
                self.position_on_map[1], self.position_on_map[0], color="black", s=25
            )
            ax.arrow(
                self.position_on_map[1],
                self.position_on_map[0],
                np.sin(self.orientation_on_map) * 10,
                np.cos(self.orientation_on_map) * 10,
                color="grey",
            )
            ...
            if not np.any(self.goal_on_map == None):
                ax.scatter(
                    self.goal_on_map[1], self.goal_on_map[0], color="white", s=25
                )
                ax.arrow(
                    self.goal_on_map[1],
                    self.goal_on_map[0],
                    np.sin(self.goal_orientation_on_map) * 10,
                    np.cos(self.goal_orientation_on_map) * 10,
                    color="green",
                )
