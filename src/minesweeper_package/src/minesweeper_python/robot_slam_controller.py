...
class RobotSlamController(object):
    ...
    def start_ros(self):
        ...
        self.map_sub = ros.Subscriber(
            self.map_sub_name,
            OccupancyGrid,
            callback=self.__map_ros_sub,
            queue_size=self.queue_size,
        )
    ...
    def send_goal(self):
        ...
        goal.target_pose.pose.position.x = (
            self.goal_on_map[0] * self.map_res + self.map_origin[0]
        )
        goal.target_pose.pose.position.y = (
            self.goal_on_map[1] * self.map_res + self.map_origin[1]
        )
        # Using the ideal "straight line" orientation computed earlier as the goal orientation
        goal_orientation = tf.transformations.quaternion_about_axis(
            self.goal_orientation_on_map, (0, 0, 1)
        )
    ...
    def __update_map_visited(self):
        ...
        if np.any(self.previous_on_map == None):
            self.map_visited[
                np.sum((coordinates - self.position_on_map) ** 2, -1)
                < (self.robot_radius / self.map_res) ** 2
            ] = True
        else:
            n_points = 10
            ...
            for i in range(n_points):
                ...
                self.map_visited[
                    np.sum((coordinates - point) ** 2, -1)
                    < (self.robot_radius / self.map_res) ** 2
                ] = True

        # If the map itself has changed --> Apply corrections as well
        if self.map_visited.shape != self.map.shape or np.any(
    ...
    def __get_pose_on_map(self):
        ...
        try:
            translation, rotation = self.transformer.lookupTransform(
                "/map", "base_footprint", ros.Time.now()
            )
            self.previous_on_map = self.position_on_map
            self.position_on_map = (
                np.array(translation[0:2]) - self.map_origin
            ) / self.map_res
            _, _, self.orientation_on_map = tf.transformations.euler_from_quaternion(
                rotation
            )
            return True
        except (
