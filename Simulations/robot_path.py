...
def plot_trajectory(world, filename, simulation_time, mines):
    ...
    if world == 3:
        ...
        edges = np.array(
            [[0, 0], [-2, 0], [-2, 2.75], [0, 2.75], [0, 6], [7, 6], [7, 0], [0, 0]]
        )
        tree = np.array([5.5, 4])
        cones_x = 2 * np.ones(4)
    ...
    coords = pd.read_csv(
        os.path.join(root, "src/minesweeper_package/log/", file_name_traj), sep=";"
    )
    ...
    if mines:
        ...
        mine_coords = pd.read_csv(
            os.path.join(root, "src/minesweeper_package/log/", file_name_mines), sep=";"
        )
        x_mine = mine_coords["mine_x"].values
        y_mine = mine_coords["mine_y"].values

        file_name_mines = filename[:20] + "distribution.txt"
        mine_distr = pd.read_csv(
            os.path.join(root, "src/minesweeper_package/log/", file_name_mines), sep=";"
        )
    ...
    # plot the trajectory
    ax.plot(
        x[time < simulation_time], y[time < simulation_time], c="blue", linewidth=0.5
    )
