...
root_path = os.path.expanduser(os.path.join(os.environ.get("ROBOTICS_PROJECT_DIR"),"Simulations/MediumWorld/Smart_Far"))
...
root_path = os.path.expanduser(os.path.join(os.environ.get("ROBOTICS_PROJECT_DIR"),"Simulations/MediumWorld/Random_Roomba"))
dir = os.listdir(root_path)
dir = [file for file in dir if file[20:] == "mines.txt"]
...
ax.plot(x, exponential(x, a,b,c), '-', label=f"{a:.2f} exp({-b:.2f}x){c:.2f}", color='blue', linewidth=2)
...
ax.plot(x, exponential(x, a,b,c), '-', label=f"{a:.3f}exp({-b:.3f}x){c:.3f}", color='black', linewidth=2)
